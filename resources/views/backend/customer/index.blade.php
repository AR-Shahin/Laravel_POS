...
@section('main_content')
        ...
        <div class="col-12 col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="text-info d-inline">Manage Customer</h4>
                    <button type="button" class="btn btn-primary pull-right" data-toggle="modal" data-target="#addModal">
                        <i class="fa fa-plus-circle mr-1"></i> Add New Customer
                    </button>
                </div>
                <div class="card-body">
                    <div class="adv-table">
    ...
    <script>
        ...
        function getAllCustomer() {
            $.ajax({
                url : <?= json_encode(route('customer.fetch')) ?>,
                method : 'GET',
                data : {},
        ...
        $('#addCustomerForm').on('submit',function (e) {
            ...
            if((name != '') && (email != '') && (address != '') && (phone != '')){
                $.ajax({
                    url : <?= json_encode(route('customer.store')) ?>,
                    method : 'POST',
        ...
        $('body').on('click','#deleteRow',function (e) {
            ...
            swalWithBootstrapButtons.fire({
                ...
                $.ajax({
                    url : <?= json_encode(route('customer.delete'))?>,
                    type : 'DELETE',
                    data : {id : id},
                    success : function (response) {
                        getAllCustomer();
        ...
        $('body').on('click','#editRow',function (e) {
            ...
            var id = $(this).attr('data-id');
            $.ajax({
                url : <?= json_encode(route('customer.edit'))?>,
                type : 'GET',
                data : {id : id},
                success : function (response) {
        ...
        $('#updateCustomerForm').on('submit',function (e) {
            e.preventDefault();
            $.ajax({
                url : <?= json_encode(route('customer.update'))?>,
                method:'PUT',
                data: $(this).serialize(),
                success:function (response) {
